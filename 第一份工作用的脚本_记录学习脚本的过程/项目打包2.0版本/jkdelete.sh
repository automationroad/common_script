...
projectname=`cat /mnt/d/bin/zero.xml | awk '{print $1}'`
	...
	do
		...
		find ${eclipse}${name}/internetware/ -maxdepth 1 -name *[^0XY]${projectpath}* | grep ${projectpath}
		if [ $? -eq 0 ]; then
			...
			sum=`egrep *${projectpath}* /mnt/d/temp/${projectpath}project.xml | grep 'internetware' | head -n 1 | grep -o ${projectpath} | grep -c ${projectpath}`
			if [ ${sum} -eq 2 ];then
				...
				egrep *${projectpath}* /mnt/d/temp/${projectpath}project.xml | grep 'internetware' | head -n 1 | sed 's/.*ware\\//g' | sed 's/\\api.*//g' | xargs -I {} mv -f ${eclipse}${name}/internetware/{} /mnt/d/temp/internetware/${name}
			else
				...
				#egrep *${projectpath}* /mnt/d/temp/${projectpath}project.xml | grep 'internetware' | grep 'internetware' | sed 's/.*>\\//g' | sed 's/<.*//' | sed 's/\\/\//g' | xargs -I {} mv -f ${eclipse}${name}/{} /mnt/d/temp/internetware/
			fi
		fi
	done
fi
...
projectname1=`cat /mnt/d/bin/nozero.xml | awk '{print $1}'`
	...
	do
		...
		#备份internetware
		find ${eclipse}${name}/internetware/ -maxdepth 1 -name *${projectpath1}* | xargs -I {} mv -f {} /mnt/d/temp/internetware/${name1}
	done
fi

...
#进入更新路径目录并开始检查操作
cd ${updatepath}
updatenum=`ls | grep -v ".*zip" | grep -v ".*rar" | grep -v ".*xlsx" | grep -v ".*txt" | grep 2017 | wc -l`
updatename=`ls | grep -v ".*zip" | grep -v ".*rar" | grep -v ".*xlsx" | grep -v ".*txt" | grep 2017`
if [ ${updatenum} -gt 0 ];then
	for path in ${updatename}
		do
			cd ./${path}/
			#新增的处理
			xz=`ls | grep -v ".*xlsx" | grep -v ".*txt" | grep '新增'`
			if test -n "${xz}"
			#直接新增，不对接口种类划分
			then 
				#接口处理
			...
			#修改的处理
			xg=`ls | grep '修改' | grep -v ".*xlsx" | grep -v ".*txt"`
			if test -n "${xg}"
			then
				#处理修改接口
				jkname=`ls ./${xg}/接口/`
				...
				do
					echo "bxkc${newdsq}" >> /mnt/c/Users/User/Desktop/dsqtemp.txt
					cp -rf ./${xg}/入库/${newdsq}/impl/* ${dsqpath}bxkc${newdsq}${impl}
					#去除空白行首行尾字符串
					sed -i 's/^[ \t]*//g' ${dsqpath}bxkc${newdsq}/config/services.xml
					sed -i 's/^[ \t]*//g' ./${xg}/入库/${newdsq}/services.xml
					sed 's/[ \t]*$//g' ${dsqpath}bxkc${newdsq}/config/services.xml
					sed 's/[ \t]*$//g' ./${xg}/入库/${newdsq}/services.xml
					#对比不同的文本行以及写入
					grep -vwf ${dsqpath}bxkc${newdsq}/config/services.xml ./${xg}/入库/${newdsq}/services.xml > /mnt/d/Workspace/test.xml
					sed -i "/<services>/r /mnt/d/Workspace/test.xml" ${dsqpath}bxkc${newdsq}/config/services.xml
				done
				fi
			fi
			#回到未更新这个目录

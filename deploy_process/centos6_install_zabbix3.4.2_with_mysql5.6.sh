#!/bin/bash
#参考博客 https://blog.csdn.net/mr_rsq/article/details/78457587
#防火墙和SELinux要关闭
#下载并解压
wget -O zabbix-3.4.7.tar.gz  https://sourceforge.net/projects/zabbix/files/ZABBIX%20Latest%20Stable/3.4.7/zabbix-3.4.7.tar.gz/download
...
sed -i '4a innodb_file_per_table=1' /etc/my.cnf
#安装php5.6和Apache服务
rpm -ivh http://repo.webtatic.com/yum/el6/latest.rpm
yum -y install httpd php56w php56w-gd php56w-mysqlnd php56w-bcmath php56w-mbstring php56w-xml php56w-ldap php56w-fpm
#编辑php的ini文件
cp /etc/php.ini /etc/php.ini.bak
sed -i '663s/8/16/g' /etc/php.ini
sed -i '375s/30/300/g' /etc/php.ini
sed -i '385s/60/300/g' /etc/php.ini
sed -i '889c date.timezone = Asia/Shanghai' /etc/php.ini
sed -i '705c always_populate_raw_post_data = -1' /etc/php.ini
#配置/etc/httpd/conf/httpd.conf
cp /etc/httpd/conf/httpd.conf /etc/httpd/conf/httpd.conf.bak
sed -i '276a ServerName 127.0.0.1' /etc/httpd/conf/httpd.conf
sed -i '293s/html/html\/zabbix/g' /etc/httpd/conf/httpd.conf
sed -i '318s/html/html\/zabbix/g' /etc/httpd/conf/httpd.conf
sed -i '403c DirectoryIndex index.html index.html.var index.php' /etc/httpd/conf/httpd.conf
#设置web前端
mkdir /var/www/html/zabbix
cp -a ./zabbix-3.4.7/frontends/php/* /var/www/html/zabbix/
#修改图形的汉字显示为方格乱码
rpm -ivh ftp://ftp.pbone.net/mirror/ftp5.gwdg.de/pub/opensuse/repositories/home:/cathay4t:/misc-rhel6/CentOS_CentOS-6/noarch/wqy-microhei-fonts-0.2.0-1.1.noarch.rpm
cp -p /var/www/html/zabbix/fonts/DejaVuSans.ttf /var/www/html/zabbix/fonts/DejaVuSans.ttf.bak
cp -f /usr/share/fonts/wqy-microhei/wqy-microhei.ttc /var/www/html/zabbix/fonts/DejaVuSans.ttf
#设置apache的执行和所有者
chown -R apache:apache /var/www/html/zabbix
#赋予可执行权限
chmod +x /var/www/html/zabbix/conf/
#安装zabbix依赖环境
yum -y install gcc* mysql-devel libxml2-devel net-snmp* java* curl-devel pcre* unixODBC-devel.x86_64 libssh2-devel.x86_64 OpenIPMI-devel.x86_64 libevent-devel.x86_64
#编译zabbix
cd ./zabbix-3.4.7
...
sed -i '68s/# //g' /usr/local/zabbix/etc/zabbix_server.conf
sed -i '85s/# //g' /usr/local/zabbix/etc/zabbix_server.conf
sed -i '126c DBSocket=/var/lib/mysql/mysql.sock' /usr/local/zabbix/etc/zabbix_server.conf
sed -i '584c Include=/usr/local/zabbix/etc/zabbix_server.conf.d/*.conf' /usr/local/zabbix/etc/zabbix_server.conf
#修改配置文件zabbix_agentd.conf
cp /usr/local/zabbix/etc/zabbix_agentd.conf /usr/local/zabbix/etc/zabbix_agentd.conf.bak
sed -i '30c LogFile=/usr/local/zabbix/logs/zabbix_agentd.log' /usr/local/zabbix/etc/zabbix_agentd.conf
sed -i '11s/# //g' /usr/local/zabbix/etc/zabbix_agentd.conf
sed -i '261c Include=/usr/local/zabbix/etc/zabbix_agentd.conf.d/*.con' /usr/local/zabbix/etc/zabbix_agentd.conf
sed -i '145s/Zabbix server/127.0.0.1/g' /usr/local/zabbix/etc/zabbix_agentd.conf
...
sed -i '22c BASEDIR=/usr/local/zabbix' /etc/init.d/zabbix_server
sed -i '22c BASEDIR=/usr/local/zabbix' /etc/init.d/zabbix_agentd
#创建zabbix用户
...
mysql -uzabbix -pzabbix zabbix < ./zabbix-3.4.7/database/mysql/schema.sql
mysql -uzabbix -pzabbix zabbix < ./zabbix-3.4.7/database/mysql/images.sql
mysql -uzabbix -pzabbix zabbix < ./zabbix-3.4.7/database/mysql/data.sql
#启动所有服务，并设置开机自启
service httpd start
